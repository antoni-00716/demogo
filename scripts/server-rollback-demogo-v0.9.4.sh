...
if [[ -z "${BACKUP_DIR}" || ! -d "${BACKUP_DIR}" ]]; then
  echo "Usage: $0 /var/lib/demogo/backups/pre-v0.9.3-YYYYmmddHHMMSS"
  exit 1
fi
...
if [[ -f /etc/systemd/system/demogo-server.service.d/runtime.conf ]]; then
  mv /etc/systemd/system/demogo-server.service.d/runtime.conf /etc/systemd/system/demogo-server.service.d/runtime.conf.disabled
fi
