...
if [[ -z "${BACKUP_DIR}" || ! -d "${BACKUP_DIR}" ]]; then
  echo "Usage: $0 /var/lib/demogo/backups/pre-v0.1.7-YYYYmmddHHMMSS"
  exit 1
fi
...
if [[ -d "${BACKUP_DIR}/data" ]]; then
  echo "Restoring JSON data..."
  rm -rf /var/lib/demogo/data
  cp -a "${BACKUP_DIR}/data" /var/lib/demogo/data
fi

echo "Removing database environment override..."
rm -f /etc/systemd/system/demogo-server.service.d/database.conf
cat >/etc/systemd/system/demogo-server.service.d/description.conf <<'EOF'
[Unit]
Description=DemoGo v0.1.6 API
EOF
