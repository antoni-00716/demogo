...
SERVER_ZIP="/tmp/demogo-server-v0.9.3.zip"
DB_ENV="/root/demogo-db.env"
DATA_DIR="/var/lib/demogo/data"
BACKUP_DIR="/var/lib/demogo/backups/pre-v0.9.3-$(date +%Y%m%d%H%M%S)"
NGINX_CONF_FILES=(
...
patch_nginx_demo_routes() {
  ...
  for conf in "${NGINX_CONF_FILES[@]}"; do
    ...
    local backup="${conf}.bak-v0.9.3-$(date +%Y%m%d%H%M%S)"
    cp -a "${conf}" "${backup}"
    "${python_cmd}" - "$conf" <<'PY'
import re
import sys
...
patch_nginx_proxy_timeouts() {
  ...
  for conf in "${NGINX_CONF_FILES[@]}"; do
    ...
    local backup="${conf}.timeouts-v0.9.3-$(date +%Y%m%d%H%M%S)"
    cp -a "${conf}" "${backup}"
...
if ! command -v docker >/dev/null 2>&1; then
  echo "Docker is required for DemoGo v0.9.3 Node.js runtime."
  exit 1
fi
