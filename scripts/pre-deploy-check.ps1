...
Check-Step 10 "Version sync" {
    ...
    $pkgs = @("server", "cli", "mcp", "web"); $verFiles = @("$root\server\VERSION")
    $ok = $true
    foreach ($pkg in $pkgs) {
    ...
    foreach ($vf in $verFiles) { if (Test-Path $vf) { $sv = (Get-Content $vf -Raw).Trim(); if ($sv -ne $verFile) { Write-Host "  MISMATCH: server/VERSION=$sv expected=$verFile" -ForegroundColor Red; $ok = $false } } }
    if ($ok) {
        Write-Host "  All files at v$verFile" -ForegroundColor Green
        $global:LASTEXITCODE = 0
    } else {
