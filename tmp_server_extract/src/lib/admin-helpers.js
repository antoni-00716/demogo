...
import { contentReviewStatusLabel, createHttpError } from "../services/content-review-service.js";
import { deploySourceLabel } from "./deploy-helpers.js";
import { filterAutoHostableFormFields } from "./form-field-utils.js";
import { normalizeEnvKey, isPlatformEnvKey, maskSecretValue } from "./env-utils.js";
...
export function adminDemoSummary(demo) {
  ...
  return {
    ...
    architecture,
    hosting,
    hostingMode: demo.hostingMode || demo.inspection?.analysis?.hostingMode || architecture?.projectKind || hosting?.mode || "",
    hostingModeLabel: demo.hostingModeLabel || demo.inspection?.analysis?.hostingModeLabel || architecture?.projectKindLabel || hosting?.modeLabel || "",
    ...
    externalBackend: publicExternalBackend(externalBackend),
    applicationReadiness: publicApplicationReadiness(applicationReadiness),
    projectProfile: demo.projectProfile || demo.inspection?.analysis?.projectProfile || null,
    projectCategory: demo.projectCategory || demo.inspection?.analysis?.projectCategory || demo.inspection?.analysis?.projectProfile?.label || "",
...
export function createRuntimeConfigStatus(inspection = {}, runtimeEnv = {}, database = null) {
  const required = Array.from(new Set([
    ...((inspection.analysis?.projectAssessment?.environmentVariables?.required || [])),
    ...((inspection.analysis?.projectProfile?.environmentVariables?.required || [])),
...
export function summarizeDemoRisks(demo) {
  ...
  const apiCalls = Array.isArray(inspection.forms?.apiCalls) ? inspection.forms.apiCalls : [];
  const formFields = Array.isArray(inspection.forms?.formFields) ? inspection.forms.formFields : [];
  const blockedFiles = Array.isArray(inspection.files?.blockedFiles) ? inspection.files.blockedFiles : [];
  const risks = [];
  ...
  if (inspection.analysis?.hasPackageJson && !inspection.analysis?.hasBuildScript && !inspection.entries?.entryFile) {
    risks.push({
      type: "build",
      label: "缺少 build"
    });
