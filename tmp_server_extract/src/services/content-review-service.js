...
export async function createAndPersistContentReview(context) {
  ...
  const record = {
    ...
    detectedType: context.inspection?.analysis?.detectedType || "",
...
export function attachContentReviewToInspection(inspection, review) {
  ...
  const issues = [...(inspection.presentation?.issues || [])];
  const suggestions = [...(inspection.presentation?.suggestions || [])];
  if (blocked) {
  ...
  return {
    ...
    presentation: {
      ...
      userStatus: blocked ? "unsupported" : inspection.presentation?.userStatus,
      userStatusLabel: blocked ? "暂不能发布" : inspection.presentation?.userStatusLabel,
      userSummary: blocked ? contentReviewUserSummary(review) : inspection.presentation?.userSummary,
      issues,
      suggestions: Array.from(new Set(suggestions)),
      ruleReport: {
        ...(inspection.presentation?.ruleReport || {}),
        risks: [
          ...((inspection.presentation?.ruleReport || {}).risks || []),
        ...
        recommendations: [
          ...((inspection.presentation?.ruleReport || {}).recommendations || []),
          ...((blocked ? (review.findings || []).filter((finding) => finding.severity !== "notice") : []).slice(0, 5).map((finding) => finding.suggestion).filter(Boolean))
        ],
        fixPrompt: createContentReviewFixPrompt(review) || (inspection.presentation?.ruleReport || {}).fixPrompt
      }
    }
  };
}
