const fs = require('fs');
const base=fs.readFileSync('C:/Users/wei.gu/Documents/demogo/server/src/routes/admin_v029.js','utf8');
const bl=base.split('\n');
let insertIdx = -1;
for(let i=0;i<bl.length;i++){
  if(bl[i].includes('/api/admin/demos/:id/offline')){
    insertIdx = i;
    break;
  }
}
console.log('Insert before line:', insertIdx+1);
const newRoutes=[
  ...
  '          const subject = "[DemoGo] 你的套餐已升级";',
  '          const text = ["你的 DemoGo 套餐已升级为 " + planLabel + "。登录工作台使用：" + (publicBaseUrl || "https://demogo.cn") + "/dashboard","如有问题请联系客服 QQ：304598006 或邮箱 hello@demogo.cn","-- DemoGo 团队"].join("\\n\\n");',
...
const result = bl.slice(0, insertIdx).concat(newRoutes).concat(bl.slice(insertIdx));
fs.writeFileSync('C:/Users/wei.gu/Documents/demogo/server/src/routes/admin.js', result.join('\n'), 'utf8');
let d=0,p=0;
for(let i=0;i<result.length;i++){for(let c of result[i]){if(c=='{')d++;if(c=='}')d--;if(c=='(')p++;if(c==')')p--;}}
console.log('Result: B='+d+' P='+p+' lines='+result.length);
