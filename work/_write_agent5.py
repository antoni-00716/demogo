...
export function AgentPublishPanel({
  ...
  const hasInstruction = !!generatedInstruction;
    ...
    <div className="workspace-home">
      ...
      <div className="agent-generate-area">
        <Button variant="primary" onClick={handleGenerate} disabled={!canGenerate}>
          <Wand2 size={16} /> {hasInstruction ? "重新生成指令" : "生成发布指令"}
        </Button>
        {!canGenerate && publishMode === "update" && (
          <span className="agent-generate-hint">请先填写原项目链接</span>
        )}
        {hasInstruction && (
          <span className="agent-generate-hint">
            <CheckCircle2 size={14} style={{ color: "var(--green-500)" }} /> 指令已生成
          </span>
        )}
      </div>
      ...
      {hasInstruction ? (
        ...
        /* 未生成指令时的引导占位 */
        <div className="agent-instruction-placeholder">
          <div className="agent-placeholder-icon">
            <Wand2 size={28} />
          </div>
          <p>点击上方的"生成发布指令"，系统会把口令和发布方式组合成一段完整的指令</p>
          <p className="agent-placeholder-sub">支持 Cursor · Windsurf · Codex · Claude Code 等 AI 编程工具</p>
        </div>
      )}
    </div>
  );
