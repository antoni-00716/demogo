...
### 目录重构
- codex-skill/ 合并入 codex-plugin/（插件已包含 skill）
- rand/logos/assets/ 扁平化 → rand/
- server/scripts/ 删除（3个Redis测试脚本）
- samples/*/dist/ + samples/*/.demogo/ 清理（30个残留目录）
- web/playwright-report/ 	est-results/ dist/ 清理
- shared/ 新增 package.json，成为正式模块
- server/package.json 修复断裂的 sync-version 引用
- .gitignore 补充 web 测试产物规则
...
### Fixed
- **降低 runtimeMaxInstances 默认值**：从 10 降至 2，防止 2G 内存服务器 OOM（可通过环境变量覆盖）
- **部署更新流程增加回滚机制**：更新 Demo 时先备份旧文件再解压，解压失败自动恢复

### Changed
- **提取重复工具函数**：新建 server/src/lib/utils.js，统一 exists、sleep、ormatBytes，消除 4 个文件中的重复定义

---

## v0.9.31 项目清理 (2026-06-02)

### 文档整合
- docs/product/ 10 个 PPT 文件合并为 1 个 opc-pitch-deck.md
- samples/README.md 与 DEMOS_SUMMARY.md 合并
- 删除 ARCHITECTURE.md（内容并入 AGENTS.md）
- 删除 ROADMAP.md（方向并入 README.md，历史在 CHANGELOG）
- OPERATIONS.md 并入 DEPLOYMENT.md
- README.md 精简，去除与 AGENTS.md 的重复

### 代码清理
- 删除 42 个 _fix_*.py 调试脚本
- 删除 cr-debug.mjs、cr-diag.mjs、diag-test.mjs 诊断脚本
- 删除 sync-version.js（保留 sync-version.ps1）
- 删除 web/src/assets/vite.svg（Vite 模板残留）
- 删除 ssets/ 空目录
- 删除 	ests/ 空目录

### 文档更新
- AGENTS.md：services 21→23、routes 10→11、技术债数据更新
- README.md：精简并新增后续方向
- web/README.md：重写为 DemoGo 前端说明
