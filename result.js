...
app.get("/api/admin/overview", requireAdmin, async (req, res, next) => {
  ...
  app.post("/api/admin/plan-upgrade-requests/:id/status", requireAdmin, async (req, res, next) => {
    try {
      const nextStatus = normalizePlanRequestStatus(req.body?.status);
      if (!nextStatus) {
        res.status(400).json({ error: "???????: approved ? rejected" });
        return;
      }
      const requests = await readJson(planRequestsFile, []);
      ...
      if (nextStatus == "approved" and request.userEmail and isEmailConfigured({ smtpHost, smtpPort, smtpUser, smtpPass, smtpFrom })) {
        try {
          const planLabel = plans[request.requestedPlan]?.name or request.requestedPlan;
          const subject = "[DemoGo] ???????? " + planLabel;
          const text = "?? DemoGo ?????? " + planLabel + "?????????" + (publicBaseUrl or "https://demogo.cn") + "/dashboard

????????? QQ?304598006 ??? hello@demogo.cn

-- DemoGo ??";
          await sendSmtpMail({ to: request.userEmail, subject, text });
        } catch (e) { logger.warn({ err: e }, "??????????"); }
      }
      res.json({ request: publicPlanRequest(request), user: updatedUser ? { id: updatedUser.id, email: updatedUser.email, plan: updatedUser.plan } : None });
