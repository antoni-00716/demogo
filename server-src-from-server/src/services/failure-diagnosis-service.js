...
export function classifyFailureCategory(input = {}) {
  const text = [
    ...
    input.inspection?.analysis?.projectAssessment?.support?.nextAction,
    ...(input.inspection?.analysis?.projectAssessment?.support?.missingRequirements || []),
    ...(input.inspection?.presentation?.issues || []),
    ...(input.inspection?.presentation?.unsupportedNotes || []),
    ...(input.inspection?.analysis?.projectProfile?.unsupportedReasons || [])
  ].map(cleanText).join("\n");
...
function extractEvidence({ message, inspection, runtime, database, contentReview, externalBackend, logs, databaseError }) {
  return [
    message,
    inspection?.analysis?.detectedType ? `项目类型：${inspection.analysis.detectedType}` : "",
    inspection?.analysis?.hostingMode ? `托管方式：${inspection.analysis.hostingMode}` : "",
    inspection?.analysis?.projectProfile?.summary ? `项目概况：${inspection.analysis.projectProfile.summary}` : "",
...
function extraUserActions(category, { inspection, runtime, externalBackend }) {
  ...
  const missing = [
    ...
    ...(inspection?.analysis?.projectAssessment?.support?.missingRequirements || [])
  ].filter(Boolean);
  ...
  if (inspection?.analysis?.projectAssessment?.support?.nextAction) {
    result.push(inspection.analysis.projectAssessment.support.nextAction);
  }
  return result;
}

function createAiFixPrompt({ category, message, inspection, runtime, database, externalBackend, logs, databaseError, userActions }) {
  const profile = inspection?.analysis?.projectProfile?.summary || inspection?.analysis?.projectCategory || "";
