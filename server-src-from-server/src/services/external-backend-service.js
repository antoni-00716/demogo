...
import { normalizeEnvKey, isPlatformEnvKey } from "../lib/env-utils.js";

export function hasSupabaseProject(inspection = {}) {
  const profile = inspection.analysis?.projectProfile || {};
  const assessment = inspection.analysis?.projectAssessment || profile.assessment || {};
  const databaseItems = [
    ...(profile.databases || []),
...
function preferredSupabaseKeys(inspection = {}) {
  ...
  const frameworks = [
    inspection.analysis?.projectProfile?.framework,
    ...(inspection.analysis?.projectProfile?.frontendFrameworks || []).map((item) => item.code)
  ...
  if (inspection.analysis?.hostingMode === "node_runtime" && !frameworks.length) {
    return {
      urlKey: "SUPABASE_URL",
...
function inferSupabaseFeatures(inspection = {}) {
  const text = [
    ...(inspection.analysis?.projectProfile?.signals || []),
    ...(inspection.analysis?.projectAssessment?.environmentVariables?.required || []),
    ...(inspection.analysis?.projectProfile?.environmentVariables?.required || []),
    inspection.analysis?.projectProfile?.summary || ""
  ].join(" ").toLowerCase();
  return {
