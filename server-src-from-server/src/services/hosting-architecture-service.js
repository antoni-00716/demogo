...
export function createHostingPlan(inspection = {}, config = {}) {
  const detectedType = String(inspection.analysis?.detectedType || "");
  const hasBackend = Boolean(inspection.analysis?.hasBackend);
  const hasSsr = Boolean(inspection.analysis?.hasSsr);
  ...
  const usesSupabaseExternalBackend = profileUsesSupabase(inspection.analysis?.projectProfile || {}) || inspection.externalBackend?.provider === "supabase";
...
function hasSupabaseProject(inspection = {}) {
  return profileUsesSupabase(inspection.analysis?.projectProfile || {}) || inspection.externalBackend?.provider === "supabase";
}
