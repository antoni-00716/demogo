...
// Each replace* function does UPSERT (INSERT ... ON DUPLICATE KEY UPDATE) inside a transaction.
...
// === UPSERT helper: replaces DELETE ALL + INSERT ALL with INSERT ... ON DUPLICATE KEY UPDATE ===
function upsertSql(columns) {
  const updateCols = columns.slice(1).map((c) => `${c}=VALUES(${c})`).join(", ");
  ...
  return withWriteLock("users", async () => {
    await transaction(async (connection) => {
      for (const user of Array.isArray(users) ? users : []) {
        ...
        ` + upsertSql(["id","email","phone","password_hash","role","status","plan_code","created_at","updated_at","last_login_at","metadata_json"]), [
          user.id,
          user.email,
          user.phone || null,
    ...
    await transaction(async (connection) => {
      for (const session of Array.isArray(sessions) ? sessions : []) {
        await connection.execute(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        ` + upsertSql(["token","user_id","created_at","expires_at","ip","user_agent"]), [
          session.token,
  ...
  return withWriteLock("demos", async () => {
    await transaction(async (connection) => {
      ...
      for (const demo of Array.isArray(demos) ? demos : []) {
        ...
        ` + upsertSql(["id","user_id","user_email","slug","name","status","public_url","current_version","project_type","detected_type","entry_file","file_count","total_bytes","source_zip_path","output_path","expires_at","created_at","updated_at","offline_at","deleted_at","visits","estimated_bytes","unique_visitors_estimate","last_visited_at","metadata_json"]), [
          demo.id,
        ...
        await connection.execute("DELETE FROM demo_versions WHERE demo_id = ?", [demo.id]);
        await insertDemoVersions(connection, demo);
        if (demo.inspection) {
          await connection.execute("DELETE FROM project_inspections WHERE demo_id = ?", [demo.id]);
  ...
  return withWriteLock("deployment_events", async () => {
    await transaction(async (connection) => {
      for (const event of Array.isArray(events) ? events : []) {
        ...
        ` + upsertSql(["id","demo_id","user_id","deployment_id","event_type","status","message","detail_json","created_at"]), [
  ...
  return withWriteLock("deployment_jobs", async () => {
    await transaction(async (connection) => {
      for (const job of Array.isArray(jobs) ? jobs : []) {
        ...
        ` + upsertSql(["id","action","actor_type","actor_id","target_type","target_id","ip","metadata_json","created_at"]), [
  ...
  return withWriteLock("feedback", async () => {
    await transaction(async (connection) => {
      for (const item of Array.isArray(items) ? items : []) {
        ...
        ` + upsertSql(["id","user_id","user_email","demo_id","demo_slug","type","message","contact","status","ip","created_at","updated_at","metadata_json"]), [
  ...
  return withWriteLock("forms", async () => {
    await transaction(async (connection) => {
      for (const item of Array.isArray(items) ? items : []) {
        ...
        ` + upsertSql(["id","user_id","user_email","demo_id","demo_slug","demo_name","public_token","name","status","fields_json","submission_count","created_at","updated_at","metadata_json"]), [
          item.id || crypto.randomUUID(),
          item.userId || item.user_id,
          item.userEmail || item.user_email || null,
          item.demoId || item.demo_id || null,
  ...
  return withWriteLock("form_submissions", async () => {
    await transaction(async (connection) => {
      for (const item of Array.isArray(items) ? items : []) {
        ...
        ` + upsertSql(["id","form_id","user_id","user_email","demo_id","demo_slug","payload_json","ip","user_agent","created_at","metadata_json"]), [
          item.id || crypto.randomUUID(),
  ...
  return withWriteLock("plan_upgrade_requests", async () => {
    await transaction(async (connection) => {
      for (const item of Array.isArray(items) ? items : []) {
        ...
        ` + upsertSql(["id","user_id","user_email","current_plan","requested_plan","status","contact","message","admin_note","handled_by","handled_at","created_at","updated_at","metadata_json"]), [
          item.id || crypto.randomUUID(),
          item.userId || item.user_id,
          item.userEmail || item.user_email || null,
  ...
  return withWriteLock("content_reviews", async () => {
    await transaction(async (connection) => {
      for (const item of Array.isArray(items) ? items : []) {
        ...
        ` + upsertSql(["id","user_id","user_email","demo_id","demo_slug","deployment_id","action","actor_type","status","provider","engine","summary","reviewed_file_count","findings_json","reviewed_files_json","project_name","file_name","detected_type","resolution_status","admin_note","handled_by","handled_at","created_at","metadata_json"]), [
          item.id || crypto.randomUUID(),
