...
describe("项目发布流程集成测试", () => {
  ...
  describe("项目检查", () => {
    ...
    it("应该成功检查dist构建产物", async () => {
      ...
      const result = await inspectProject(zipPath, user.cookie);
      assertEqual(result.status, 200, "检查应该返回200状态码");
      assert(result.data.inspection.detectedType === "dist", "应该识别为dist类型");
    ...
    it("应该成功检查build构建产物", async () => {
      ...
      const result = await inspectProject(zipPath, user.cookie);
      assertEqual(result.status, 200, "检查应该返回200状态码");
      assert(result.data.inspection.detectedType === "build", "应该识别为build类型");
    });
    ...
    it("应该成功检查out构建产物", async () => {
      ...
      const result = await inspectProject(zipPath, user.cookie);
      assertEqual(result.status, 200, "检查应该返回200状态码");
      assert(result.data.inspection.detectedType === "out", "应该识别为out类型");
    });
    ...
    it("应该正确识别Node.js后端项目", async () => {
      ...
      assertEqual(result.status, 200, "检查应该返回200状态码");
      assert(
        ["runtime", "node-runtime", "backend"].includes(result.data.inspection.detectedType),
        "应该识别为运行时类型"
      );
    });
