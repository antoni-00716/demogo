...
async function postZip(endpoint, zipPath, fields = {}, extraHeaders = {}) {
  ...
  form.append("file", new Blob([bytes], { type: mimeType }), path.basename(zipPath));
...
try {
  ...
  assert.ok(tarDeploy.data.id, "tar.gz deploy should return id");
  const tarPage = await getText(`/d/${tarDeploy.data.slug}/`);
  assert.ok(tarPage.includes("Tar Demo"), "tar.gz published page should contain original content");
  await postJson(`/api/demos/${tarDeploy.data.id}/offline`, {}, { Cookie: cookie });
  await postJson(`/api/demos/${tarDeploy.data.id}/delete`, {}, { Cookie: cookie });
  ...
  assert.ok(deployResult.data.id, "deploy should return demo id");
  const demoId = deployResult.data.id;
  const slug = deployResult.data.slug;
  assert.ok(deployResult.data.contentReviewStatus === "passed", "content review should be passed");
  assert.ok(deployResult.data.hostingMode === "static", "hosting mode should be static");
  ...
  assert.ok(demoInspection.inspection, "inspection should be available");
  ...
  assert.ok(proDeploy.data.customDomainEligible === true, "pro should be eligible");
  
  const oldSlug = proDeploy.data.slug;
  const slugUpdate = await postJson(`/api/demos/${proDeploy.data.id}/slug`, { slug: "pro-custom-link" }, { Cookie: cookie });
  ...
  const subdomainRequest = await postJson(`/api/demos/${proDeploy.data.id}/subdomain-requests`, { subdomain: "pro-custom-link" }, { Cookie: cookie });
  ...
  await postJson(`/api/demos/${proDeploy.data.id}/offline`, {}, { Cookie: cookie });
  await postJson(`/api/demos/${proDeploy.data.id}/delete`, {}, { Cookie: cookie });
