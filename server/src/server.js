...
async function handleCreateDeployment(req, res, next, options = {}) {
  ...
  try {
    ...
    if (!preInspection.canPublish) {
      const err = new Error(preInspection.summary || "项目检查未通过，请根据提示调整后重试。");
      err.statusCode = 400;
  ...
  if (globalThis.__demogoQueueAvailable) {
    ...
    res.json({ jobId: job.id, status: "queued", message: "部署任务已提交，正在处理中" });
  } else {
...
// Background sync demo files to object storage (MinIO)
