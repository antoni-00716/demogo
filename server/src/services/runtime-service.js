...
export function detectRuntimeWarnings({ dependencies = {}, scripts = {}, paths = [] }) {
  const depValues = Array.isArray(dependencies) ? dependencies : Object.keys(dependencies);
  const text = [
    depValues.join(" "),
...
export function canStartNodeRuntime(inspection = {}, config = {}) {
  ...
  const assessment = inspection.analysis?.projectAssessment || {};
  ...
  if ((inspection.analysis?.hasSsr || profile.type === "fullstack_framework") && !supportedSsrRuntime) {
    return { ok: false, reason: "当前只支持 Next.js、Nuxt、TanStack Start 这类单服务 SSR 项目，其他全栈框架暂不支持运行态。" };
  }
  if (!(inspection.analysis?.hasBackend || inspection.analysis?.detectedType === "backend" || profile.type === "node_service" || supportedSsrRuntime)) {
...
function selectRuntimeCommand(inspection = {}) {
  ...
  const framework = String(runtime.framework || inspection.analysis?.projectProfile?.framework || "").toLowerCase();
