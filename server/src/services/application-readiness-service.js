export function createApplicationReadiness(input = {}) {
  ...
  const profile = demo.projectProfile || inspection.projectProfile || {};
  const assessment = profile.assessment || inspection.projectAssessment || {};
...
function inferApplicationContext({ demo, inspection, profile, assessment, runtime, database, externalBackend }) {
  ...
  const hostingMode = demo.hostingMode || inspection.hostingMode || "";
  ...
  const hasFrontend = Boolean(
    ...
    inspection.entryFile ||
    inspection.detectedType === "single-html"
  );
  const hasBackend = Boolean(hostingMode === "node_runtime" || inspection.hasBackend || profile.backendFrameworks?.length || runtime?.engine);
...
function createBackendCheck({ inspection, profile, runtime }) {
  const hasBackend = Boolean(inspection.hasBackend || profile.backendFrameworks?.length || runtime?.engine);
  if (!hasBackend) {
    return {
      code: "backend",
      label: "后端接口",
...
function createRuntimeCheck({ demo, inspection, runtime, runtimeConfig }) {
  const needsRuntime = Boolean(demo.hostingMode === "node_runtime" || inspection.hostingMode === "node_runtime" || inspection.hasBackend || inspection.hasSsr);
  if (!needsRuntime) {
    return {
      code: "runtime",
      label: "运行环境",
