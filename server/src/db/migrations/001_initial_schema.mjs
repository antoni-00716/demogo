// DemoGo v0.9.6 ? Initial database schema
// This migration creates all core tables and seed data.
...
export async function down({ query }) {
  ...
  await query("DROP TABLE IF EXISTS forms");
  await query("DROP TABLE IF EXISTS feedback");
  await query("DROP TABLE IF EXISTS plan_upgrade_requests");
  await query("DROP TABLE IF EXISTS audit_logs");
  await query("DROP TABLE IF EXISTS content_reviews");
  await query("DROP TABLE IF EXISTS project_inspections");
  await query("DROP TABLE IF EXISTS deployment_events");
  await query("DROP TABLE IF EXISTS demo_versions");
  await query("DROP TABLE IF EXISTS demos");
  await query("DROP TABLE IF EXISTS plans");
