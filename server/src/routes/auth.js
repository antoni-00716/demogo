...
export function registerAuthRoutes(app, deps) {
  const {
    requireUser, requireAgentToken,
    emailVerificationEnabled, verificationCodeTtlMs, verificationResendMs,
    readJson, writeJson, usersFile, sessionsFile, emailVerificationsFile, demosFile,
    isEmailConfigured, normalizeEmail, hashPassword, verifyPassword,
    hashVerificationCode, verifyEmailCode, markEmailCodeUsed, sendVerificationEmail,
    createSession, setSessionCookie,
    checkLoginFailureRate, recordLoginFailure, clearLoginFailures,
    createAgentTokenRecord, publicAgentToken, getUserFromRequest,
    getClientIp, writeTrialEvent, writeAuditLog,
    publicUser, publicUserDemo, calculateQuota,
  } = deps;
  ...
  app.post("/api/auth/send-verification-code", async (req, res, next) => {
    try {if (!emailVerificationEnabled || !isEmailConfigured()) {
  ...
  app.post("/api/auth/register", async (req, res, next) => {
    try {const email = normalizeEmail(req.body?.email);
      const password = String(req.body?.password || "");
      ...
      if (emailVerificationEnabled && isEmailConfigured()) {
      ...
      const user = {
        ...
        passwordHash: await hashPassword(password).then(h => {return h; })
      };
      users.push(user);
await writeJson(usersFile, users);if (emailVerificationEnabled) await markEmailCodeUsed(email, verificationCode, "register");await writeTrialEvent({
  ...
  app.post("/api/auth/logout", async (req, res, next) => {
    try {const token = req.cookies?.demogo_session;
      if (token) {
  ...
  app.get("/api/agent-token", requireUser, async (req, res, next) => {
    try {res.json({ token: publicAgentToken(req.user) });
    } catch (error) {
      next(error);
    }
  });
  // --- /api/agent-token (POST) ---
...
await writeJson(usersFile, users);await writeAuditLog({
