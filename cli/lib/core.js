...
async function parseDeploymentResponse(response) {
  ...
  if (!response.ok) {
    const message = payload?.error || readableHttpError(response.status, text);
    const diagnosis = payload?.failureDiagnosis || payload?.inspection?.failureDiagnosis;
    if (diagnosis) {
      const details = [
        "",
        `失败类别：${diagnosis.title || diagnosis.category || "未知"}`,
        diagnosis.summary ? `原因：${diagnosis.summary}` : "",
        ...(diagnosis.evidence || []).map((e) => `证据：${e}`),
        ...(diagnosis.userActions || []).map((a) => `建议：${a}`),
        diagnosis.aiPrompt ? `\n给 AI 工具的修改建议：\n${diagnosis.aiPrompt}` : ""
      ].filter(Boolean).join("\n");
      throw new Error(`${message}${details}`);
    }
...
export async function getProjectDetails(apiBase, token, demoId, userAgent = `demogo-cli/${VERSION}`) {
  let response;
  try {
    response = await fetch(`${normalizeApiBase(apiBase)}/api/agent/project/${demoId}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
        "User-Agent": userAgent
