...
v0.9.10 事故教训：跳过集成测试导致缺少 start 脚本和 email/mailer.js 导出错误，服务无法启动。
...
- `server/src/services/` — 21 个服务（项目识别、内容审查、失败诊断、运行环境等）
...
门禁包含 9 项检查：语法检查、入口语法、单元测试、集成测试、烟雾测试、前端 lint、前端构建、npm audit、版本号统一。全部通过才能部署。
