...
负责人安排任务时，Codex 按以下流程执行：

1. **先理解任务本质**：分析任务的目标和执行方式，判断这个任务属于什么类型（如部署、开发、调试、文档、设计、调研等）。
2. **再匹配现有工具**：根据任务类型，查看当前环境已有的插件(plugins)和技能(skills)，看是否有能直接完成或辅助完成该任务的能力。
3. **然后检查插件商店**：如果当前环境没有匹配的工具，去插件商店(Plugin Store)搜索该任务可用的插件或技能。
4. **最后才是自定义开发**：只有在现有插件和技能都无法满足任务需求时，才从头编写代码或方案。

核心逻辑：**先理解，再找工具，最后动手。** 不要拿到任务就直接写代码，先想想有没有现成的轮子。
...
- **优先使用现有插件和技能**：接手任务时，先理解任务类型和目标，再根据类型匹配当前环境已有的插件和技能，然后检查插件商店，最后才考虑自定义开发。
- 所有用户可见文案要优先用非技术语言。
